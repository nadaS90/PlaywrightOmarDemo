...
Call log:
  - waiting for locator('loginpassword')
...
```ts
  ...
  4  |   // define variables - private and read only
  ...
  13 |     this.page = page;
  14 |     this.loginLink = page.getByRole("link", { name: "Log in" });
  15 |     this.usernameInput = page.locator("#loginusername");
  16 |     this.passwordInput = page.locator("loginpassword");
  17 |     this.loginBtn = page.getByRole("button", { name: "Log in" });
  18 |   }
  19 | 
  20 |   // action methods
  21 | 
  22 |   async clickLoginLink() {
  23 |     this.loginLink.click();
  24 |   }
  25 | 
  26 |   async enterUserName(username: string) {
  27 |     this.usernameInput.fill(username);
  28 |   }
  29 | 
  30 |   async enterUserPassword(password: string) {
> 31 |     this.passwordInput.fill(password);
     |                        ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  32 |   }
  33 | 
  34 |   async clickLoginBtn() {
  35 |     this.loginBtn.click();
  36 |   }
  37 | 
  38 |   async performLogin(username: string, password: string)
  39 |   {
  40 |       await this.enterUserName(username);
  41 |       await this.enterUserPassword(password);
